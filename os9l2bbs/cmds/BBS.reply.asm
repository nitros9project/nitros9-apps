...
start               stx       ReplySubjectPointer,u ; retain the parent subject parameter
                    clr       >ReservedStateByte2,u ; initialize reserved state byte2
...
ShiftNewIndexOffset aslb                ; shift b left arithmetically
                    ...
                    ldx       ReplySubjectPointer,u ; recover reply subject pointer
                    leay      >ReplySubject,u ; select reply subject
                    ldb       #30       ; initialize  to 30
CopyReplySubject    lda       ,x+       ; consume the next byte while copy reply subject
                    sta       ,y+       ; retain
                    cmpa      #13       ; recognize the carriage-return terminator
                    beq       ReplySubjectCopied ; select reply subject copied when the requested case matches
                    decb                ; decrement b
                    bne       CopyReplySubject ; repeat copy reply subject until the terminating condition is met
ReplySubjectCopied  leax      >AliasPath,pc ; select alias path
...
PromptForRecipient  leax      >RecipientPrompt,pc ; ask who should receive the reply
                    ldy       #36       ; cap this output request at 36 bytes
                    lda       #1        ; select standard output
                    os9       I$Write   ; write the requested fixed-size field
                    leax      >RecipientInput,u ; select recipient input
...
UppercaseRecipient  lda       ,x+       ; consume the next byte while uppercase recipient
                    ...
                    cmpa      #13       ; recognize the carriage-return terminator
                    bne       UppercaseRecipient ; repeat uppercase recipient until the terminating condition is met
...
CompareAliasName    lda       ,x+       ; consume the next byte while compare alias name
                    ...
                    bra       CompareAliasName ; continue with compare alias name
...
FindAliasIdSeparator lda       ,x+       ; consume the next byte while find alias id separator
                    ...
                    bne       FinishReply ; select finish reply when the requested case does not match
                    leax      <AliasLineBuffer,u ; select alias line buffer
...
RewriteIndexHeader  pshs      u         ; preserve u across the operation
                    ...
                    lda       #3        ; request OS-9 access mode 3
                    os9       I$Open    ; open the selected OS-9 path
                    bcc       UserStatsReady ; select user stats ready when carry remains clear
...
FindUserStatsRecord leax      >StatsUserId,u ; select stats user id
                    ...
                    os9       I$Read    ; read the requested fixed-size field
                    bcs       UserStatsScanEnded ; select user stats scan ended when carry reports an error or underflow
                    ldd       >StatsUserId,u ; recover stats user id
                    cmpd      CallerUserId,u ; test against caller user id
                    bne       FindUserStatsRecord ; repeat find user stats record until the terminating condition is met
                    bra       UpdateUserPostCount ; continue with update user post count
UserStatsScanEnded  cmpb      #211      ; recognize the OS-9 end-of-file status
                    ...
                    lbra      CreateUserStatsRecord ; continue with create user stats record
UpdateUserPostCount ldd       >StatsPostCount,u ; recover stats post count
                    ...
                    bge       HaveStatsWriteOffset ; continue with have stats write offset at or above the signed limit
                    leax      -$01,x    ; select -$01
HaveStatsWriteOffset ldu       ,s        ; recover
                    ...
                    lda       UserStatsPathNum,u ; recover user stats path num
                    os9       I$Write   ; write the requested fixed-size field
                    lbra      ExitSuccessfully ; continue with exit successfully
