...
recognizes a caller it will automatically run two programs, Login and Monitor. Login will first list the
'motd' file to the caller, and then prompt him to log in. Monitor will simply monitor the carrier detect
status on the port, Once the user logs in, Login will run the program specified for that user in the
...
program is finished, login will list the 'eotd' file to the caller and hang up.
...
be rectified, however, by putting a '</1' after every command in the file. This will cause it to take its
...
bits are operating. Tsmon then automatically runs two commands, Login and Monitor. If the -m option
...
The Login program allows you to maintain a list of users and their passwords and user numbers.
...
The Login program requires several files to run. The first file it looks for is 'motd'. This file is
...
The Login program next looks for a file called BBS.users. This file contains the name, password,
...
and run the menu program. The -x stops the board from hanging up if an error occurs.

Another file that Login looks for is a file called 'Userlog'. If the file does not exist, Login creates
...
The last file that Login looks for is a file called 'eotd'. This file is displayed after the command that
...
Still another file that Login looks for is */dd/bbs/BBS.userstats*. if this file does not exist, Login
will create it. If it does exist, Login will look for the current user's entry in the file. If the current user's
entry is not found, an entry is created for him, Note that this means that you NEVER have to fool with
the userstats file, it will be maintained automatically. Login will then store the time and then run the
program you specified, When you exit from the menu, Login will update the amount of time remaining
...
calling program was Login, the user will be disconnected after receiving the 'eotd' file.
...
up. The time remaining is set up by the Login command using the value given in the BBS.users file
...
messages. This editor is provided to the sysop to add his own features to the board. The build
...
simply have as a menu option 'f BBS.build /p'. This will allow someone to type data directly onto the
printer.
...
need to be created by you. It will be automatically created by Login. This file will be updated by the
...
The AnsiCode command allows you to sand ANSI code sequences through the BBS.ANSI

codes are used to control colors etc. on ANSI terminals. Some of the more important codes are listed
below.

Code Meaning Code Meaning
Om Bold Face On 04m Underline On
...
**Note:** The chat command requires that the local terminal has at least one unused window available.
...
7 NOTE: This command requires that the file '/dd/bbs/bbs. alias' exist
### `BBS.conf`
...
message. The mail post command uses the same user friendly editor as BBS.post and BBS.build.
...
command lo set the user number, and check a specific user's mail.
...
The BBS.mail.check command allows a BBS user to see if the mail ha sent has been received,
...
B8BBS.download and need not be run by the sysop. if the filename is not specified, the user is prompted
tor one.
...
doing a 'chd' command to the new directory. The menu program will then automatically use the new
menu files.
...
user logon should run the command 'New_user. This command will get all of the person's information
...
of symbol), not a special ANSI character sequence. (arrows, set graphics, save position, etc.). If you
...
**Solution:** Make sure that the file '/dd/bbs/bbs.alias' file exists and has the proper user's aliases
